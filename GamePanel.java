...
public class GamePanel extends JPanel{	
	...
	private static BuffCreature[] buffchasers;
	private static CrookedCreature[] cChasers;
	...
	//	this.creatureCount=numCreatures;
		...
		buffchasers = new BuffCreature[numCreatures];
		cChasers = new CrookedCreature[numCreatures];
		...
		for (int i = 0; i < numCreatures+numLvl; i++){
			...
			buffchasers[i] = new BuffCreature (map, temp.nextInt(height-2)+1,
										temp.nextInt(width-2)+1);
			cChasers[i] = new CrookedCreature (map, temp.nextInt(height-2)+1,
										temp.nextInt(width-2)+1);
			}
		...
		//	the mapPanel
		mapPanel = new MapPanel (map, player, chasers,buffchasers, cChasers, bSize);
			...
			if (mapPanel.checkWinLoss() == 1){
			   	...
			   	//	cChasers= new CrookedCreature[creatureCount];			   
					...
					//randomly sets resets all Creature types
			   		...
			   		for (int i = 0; i < (buffchasers.length); i++){
			   			buffchasers[i].setNewMap(map);	//resets map for creatures
				   		buffchasers[i].shuffle(temp.nextInt(50-4)+1,temp.nextInt(70-4)+1);
			   		}
			   		for (int i = 0; i < (cChasers.length); i++){
			   			cChasers[i].setNewMap(map);		//resets map for creatures
				   		cChasers[i].shuffle(temp.nextInt(50-4)+1,temp.nextInt(70-4)+1);
			   		}
			   		mapPanel.repaint();
			...
			int answer = JOptionPane.showConfirmDialog
		   	...
		   	if (answer==JOptionPane.NO_OPTION){
				...
				player.setNewMap(map);
		   		...
		   		for (int i = 0; i < buffchasers.length; i++){
		   			buffchasers[i].setNewMap(map);
			   		buffchasers[i].shuffle(temp.nextInt(50-2)+1,temp.nextInt(70-2)+1);
		   		}
		   		for (int i = 0; i < cChasers.length; i++){
		   			cChasers[i].setNewMap(map);
			   		cChasers[i].shuffle(temp.nextInt(50-2)+1,temp.nextInt(70-2)+1);
		   		}
